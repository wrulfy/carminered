AttackAnimationPointers:
	table_width 2, AttackAnimationPointers
	dw PoundAnim
	dw KarateChopAnim
	dw DoubleSlapAnim
	dw CometPunchAnim
	dw MegaPunchAnim
	dw PayDayAnim
	dw FirePunchAnim
	dw IcePunchAnim
	dw ThunderPunchAnim
	dw ScratchAnim
	dw VicegripAnim
	dw GuillotineAnim
	dw RazorWindAnim
	dw SwordsDanceAnim
	dw CutAnim
	dw GustAnim
	dw WingAttackAnim
	dw WhirlwindAnim
	dw FlyAnim
	dw BindAnim
	dw SlamAnim
	dw VineWhipAnim
	dw StompAnim
	dw DoubleKickAnim
	dw MegaKickAnim
	dw JumpKickAnim
	dw RollingKickAnim
	dw SandAttackAnim
	dw HeatButtAnim
	dw HornAttackAnim
	dw FuryAttackAnim
	dw HornDrillAnim
	dw TackleAnim
	dw BodySlamAnim
	dw WrapAnim
	dw TakeDownAnim
	dw ThrashAnim
	dw DoubleEdgeAnim
	dw TailWhipAnim
	dw PoisonStingAnim
	dw TwineedleAnim
	dw PinMissileAnim
	dw LeerAnim
	dw BiteAnim
	dw GrowlAnim
	dw RoarAnim
	dw SingAnim
	dw SupersonicAnim
	dw SonicBoomAnim
	dw DisableAnim
	dw AcidAnim
	dw EmberAnim
	dw FlamethrowerAnim
	dw MistAnim
	dw WaterGunAnim
	dw HydroPumpAnim
	dw SurfAnim
	dw IceBeamAnim
	dw BlizzardAnim
	dw PsyBeamAnim
	dw BubbleBeamAnim
	dw AuroraBeamAnim
	dw HyperBeamAnim
	dw PeckAnim
	dw DrillPeckAnim
	dw SubmissionAnim
	dw LowKickAnim
	dw CounterAnim
	dw SeismicTossAnim
	dw StrengthAnim
	dw AbsorbAnim
	dw MegaDrainAnim
	dw LeechSeedAnim
	dw GrowthAnim
	dw RazorLeafAnim
	dw SolarBeamAnim
	dw PoisonPowderAnim
	dw StunSporeAnim
	dw SleepPowderAnim
	dw PetalDanceAnim
	dw StringShotAnim
	dw DragonRageAnim
	dw FireSpinAnim
	dw ThunderShockAnim
	dw ThunderBoltAnim
	dw ThunderWaveAnim
	dw ThunderAnim
	dw RockThrowAnim
	dw EarthquakeAnim
	dw FissureAnim
	dw DigAnim
	dw ToxicAnim
	dw ConfusionAnim
	dw PsychicAnim
	dw HypnosisAnim
	dw MeditateAnim
	dw AgilityAnim
	dw QuickAttackAnim
	dw RageAnim
	dw TeleportAnim
	dw NightShadeAnim
	dw MimicAnim
	dw ScreechAnim
	dw DoubleTeamAnim
	dw RecoverAnim
	dw HardenAnim
	dw MinimizeAnim
	dw SmokeScreenAnim
	dw ConfuseRayAnim
	dw WithdrawAnim
	dw DefenseCurlAnim
	dw BarrierAnim
	dw LightScreenAnim
	dw HazeAnim
	dw ReflectAnim
	dw FocusEnergyAnim
	dw BideAnim
	dw MetronomeAnim
	dw MirrorMoveAnim
	dw SelfdestructAnim
	dw EggBombAnim
	dw LickAnim
	dw SmogAnim
	dw SludgeAnim
	dw BoneClubAnim
	dw FireBlastAnim
	dw WaterfallAnim
	dw ClampAnim
	dw SwiftAnim
	dw SkullBashAnim
	dw SpikeCannonAnim
	dw ConstrictAnim
	dw AmnesiaAnim
	dw KinesisAnim
	dw SoftboiledAnim
	dw HiJumpKickAnim
	dw GlareAnim
	dw DreamEaterAnim
	dw PoisonGasAnim
	dw BarrageAnim
	dw LeechLifeAnim
	dw LovelyKissAnim
	dw SkyAttackAnim
	dw TransformAnim
	dw BubbleAnim
	dw DizzyPunchAnim
	dw SporeAnim
	dw FlashAnim
	dw PsywaveAnim
	dw SplashAnim
	dw AcidArmorAnim
	dw CrabHammerAnim
	dw ExplosionAnim
	dw FurySwipesAnim
	dw BonemerangAnim
	dw RestAnim
	dw RockSlideAnim
	dw HyperFangAnim
	dw SharpenAnim
	dw ConversionAnim
	dw TriAttackAnim
	dw SuperFangAnim
	dw SlashAnim
	dw SubstituteAnim
	dw StruggleAnim
	assert_table_length NUM_ATTACKS
	dw ShowPicAnim
	dw EnemyFlashAnim
	dw PlayerFlashAnim
	dw EnemyHUDShakeAnim
	dw TradeBallDropAnim
	dw TradeBallAppear1Anim
	dw TradeBallAppear2Anim
	dw TradeBallPoofAnim
	dw XStatItemAnim
	dw XStatItemAnim
	dw ShrinkingSquareAnim
	dw ShrinkingSquareAnim
	dw XStatItemBlackAnim
	dw XStatItemBlackAnim
	dw ShrinkingSquareBlackAnim
	dw ShrinkingSquareBlackAnim
	dw UnusedAnim
	dw UnusedAnim
	dw ParalyzeAnim
	dw ParalyzeAnim
	dw PoisonAnim
	dw PoisonAnim
	dw SleepPlayerAnim
	dw SleepEnemyAnim
	dw ConfusedPlayerAnim
	dw ConfusedEnemyAnim
	dw FaintAnim
	dw BallTossAnim
	dw BallShakeAnim
	dw BallPoofAnim
	dw BallBlockAnim
	dw GreatTossAnim
	dw UltraTossAnim
	dw ShakeScreenAnim
	dw HidePicAnim
	dw ThrowRockAnim
	dw ThrowBaitAnim
	assert_table_length NUM_ATTACK_ANIMS
	dw ZigZagScreenAnim

; each animation is a list of subanimations
; and/or special effects, terminated by -1

;\1: sound_id
;\2: special_effect_id or subanimation_id
; if \2 is a subanimation_id:
;\3: tileset_id
;\4: delay
battle_anim: MACRO
	IF _NARG == 4
		db (\3 << 6) | \4
		db \1 - 1
		db \2
	ELSE
		db \2
		db \1 - 1
	ENDC
ENDM

ZigZagScreenAnim:
	battle_anim NO_MOVE, SE_WAVY_SCREEN
	db -1 ; end

PoundAnim:;ok
	battle_anim POUND, SUBANIM_01, 0, 8
	db -1 ; end

StruggleAnim:;ok
	battle_anim SPLASH, SE_BOUNCE_UP_AND_DOWN
	battle_anim POUND, SUBANIM_01, 0, 4
	db -1 ; end

KarateChopAnim:;ok
	battle_anim KARATE_CHOP, SUBANIM_03, 0, 8
	db -1 ; end

DoubleSlapAnim:;ok
	battle_anim DOUBLESLAP, SUBANIM_01, 0, 5
	battle_anim DOUBLESLAP, SUBANIM_01, 0, 5
	db -1 ; end

CometPunchAnim:;ok
	battle_anim COMET_PUNCH, SUBANIM_02, 0, 4
	battle_anim COMET_PUNCH, SUBANIM_02, 0, 4
	db -1 ; end

MegaPunchAnim:;ok
	battle_anim MEGA_PUNCH, SUBANIM_04, 1, 6
	db -1 ; end

PayDayAnim:;ok
	battle_anim POUND, SUBANIM_01, 0, 8
	battle_anim PAY_DAY, SUBANIM_52, 0, 4
	db -1 ; end

FirePunchAnim:;ok
	battle_anim FIRE_PUNCH, SUBANIM_02, 0, 6
	battle_anim NO_MOVE, SUBANIM_11, 1, 6
	db -1 ; end

IcePunchAnim:;ok
	battle_anim ICE_PUNCH, SUBANIM_02, 0, 6
	battle_anim NO_MOVE, SUBANIM_2F, 0, 16
	db -1 ; end

ThunderPunchAnim:;ok
	battle_anim THUNDERPUNCH, SUBANIM_02, 0, 6
	battle_anim NO_MOVE, SE_DARK_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_2B, 1, 6
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

ScratchAnim:;ok
	battle_anim SCRATCH, SUBANIM_0F, 0, 6
	db -1 ; end

VicegripAnim:;ok
	battle_anim VICEGRIP, SUBANIM_2A, 0, 8
	db -1 ; end

GuillotineAnim:;ok
	battle_anim SWORDS_DANCE, SUBANIM_18, 1, 6
	battle_anim GUILLOTINE, SUBANIM_2A, 0, 6
	db -1 ; end

RazorWindAnim:;done
	battle_anim GUST, SUBANIM_10, 1, 4
	battle_anim RAZOR_WIND, SUBANIM_16, 0, 4
	db -1 ; end

SwordsDanceAnim:;ok
	battle_anim SWORDS_DANCE, SUBANIM_18, 1, 6
	battle_anim SWORDS_DANCE, SUBANIM_18, 1, 6
	battle_anim SWORDS_DANCE, SUBANIM_18, 1, 6
	db -1 ; end

CutAnim:;ok
	battle_anim CUT, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SUBANIM_16, 0, 4
	db -1 ; end

GustAnim:;ok
	battle_anim GUST, SUBANIM_10, 1, 6
	battle_anim NO_MOVE, SUBANIM_02, 0, 6
	db -1 ; end

WingAttackAnim:;done
	battle_anim WING_ATTACK, SUBANIM_59, 0, 6
	db -1 ; end

WhirlwindAnim:;done
	battle_anim WHIRLWIND, SUBANIM_10, 1, 6
	battle_anim NO_MOVE, SE_SLIDE_ENEMY_MON_OFF
	battle_anim NO_MOVE, SE_SHOW_ENEMY_MON_PIC
	db -1 ; end

FlyAnim:; ok
	battle_anim FLY, SUBANIM_04, 1, 6
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	db -1 ; end

BindAnim:; done
	battle_anim VINE_WHIP, SUBANIM_16, 0, 1
	battle_anim BIND, SUBANIM_23, 0, 4
	battle_anim BIND, SUBANIM_23, 0, 4
	db -1 ; end

SlamAnim:; ok
	battle_anim SLAM, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_RESET_MON_POSITION
	battle_anim NO_MOVE, SUBANIM_02, 0, 6
	db -1 ; end

VineWhipAnim:;ok
	battle_anim VINE_WHIP, SUBANIM_16, 0, 1
	battle_anim NO_MOVE, SUBANIM_01, 0, 8
	db -1 ; end

StompAnim:;ok
	battle_anim STOMP, SUBANIM_05, 1, 8
	db -1 ; end

DoubleKickAnim:; ok
	battle_anim DOUBLE_KICK, SUBANIM_01, 0, 8
	battle_anim DOUBLE_KICK, SUBANIM_01, 0, 8
	db -1 ; end

MegaKickAnim:;done
	battle_anim LEECH_SEED, SE_SLIDE_MON_HALF_OFF
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_RESET_MON_POSITION
	battle_anim MEGA_KICK, SUBANIM_04, 1, 8
	db -1 ; end

JumpKickAnim:; very done
	battle_anim MEDITATE, SE_SQUISH_MON_PIC
	battle_anim NO_MOVE, SE_SHOOT_BALLS_UPWARD
	battle_anim JUMP_KICK, SUBANIM_03, 0, 4
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	db -1 ; end

RollingKickAnim:; ok?
	battle_anim NO_MOVE, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_RESET_MON_POSITION
	battle_anim ROLLING_KICK, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SUBANIM_04, 1, 6
	db -1 ; end

SandAttackAnim:; ok
	battle_anim SAND_ATTACK, SUBANIM_28, 1, 6
	db -1 ; end

HeatButtAnim:;ok
	battle_anim HEADBUTT, SUBANIM_05, 1, 6
	db -1 ; end

HornAttackAnim:;ok
	battle_anim HORN_ATTACK, SUBANIM_45, 0, 6
	battle_anim NO_MOVE, SUBANIM_05, 1, 6
	db -1 ; end

FuryAttackAnim:; done
	battle_anim FURY_ATTACK, SUBANIM_46, 0, 2
	battle_anim NO_MOVE, SUBANIM_46, 0, 2
	db -1 ; end

HornDrillAnim:; very done
	battle_anim LEECH_SEED, SE_DARK_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SPIRAL_BALLS_INWARD
	battle_anim HORN_DRILL, SUBANIM_58, 0, 1
	battle_anim HORN_DRILL, SUBANIM_58, 0, 2
	battle_anim HORN_DRILL, SUBANIM_58, 0, 3
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

TackleAnim:; ok
	battle_anim LEECH_SEED, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_RESET_MON_POSITION
	db -1 ; end

BodySlamAnim:;ok
	battle_anim LEECH_SEED, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_MON_POSITION
	db -1 ; end

WrapAnim:;ok
	battle_anim WRAP, SUBANIM_23, 0, 4
	battle_anim WRAP, SUBANIM_23, 0, 4
	battle_anim WRAP, SUBANIM_23, 0, 4
	db -1 ; end

TakeDownAnim:;ok
	battle_anim LEECH_SEED, SE_MOVE_MON_HORIZONTALLY
	battle_anim TAKE_DOWN, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_MON_POSITION
	db -1 ; end

ThrashAnim:;done
	battle_anim THRASH, SUBANIM_03, 0, 3
	battle_anim THRASH, SUBANIM_02, 0, 4
	battle_anim THRASH, SUBANIM_04, 1, 6
	db -1 ; end

DoubleEdgeAnim:;fixed
	battle_anim LEECH_SEED, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_2D, 0, 6
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_MOVE_MON_HORIZONTALLY
	battle_anim DOUBLE_EDGE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_MON_POSITION
	db -1 ; end

TailWhipAnim:;ok
	battle_anim AMNESIA, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim AMNESIA, SE_RESET_MON_POSITION
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim AMNESIA, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim AMNESIA, SE_RESET_MON_POSITION
	db -1 ; end

PoisonStingAnim:;ok
	battle_anim POISON_STING, SUBANIM_00, 0, 6
	db -1 ; end

TwineedleAnim:;done
	battle_anim TWINEEDLE, SUBANIM_57, 0, 4
	battle_anim NO_MOVE, SUBANIM_01, 0, 3
	db -1 ; end

PinMissileAnim:;very done
	battle_anim PIN_MISSILE, SUBANIM_57, 0, 2
	battle_anim NO_MOVE, SUBANIM_01, 0, 2
	db -1 ; end

LeerAnim:;okay
	battle_anim LEECH_SEED, SE_DARK_SCREEN_PALETTE
	battle_anim LEER, SE_DARK_SCREEN_FLASH
	battle_anim LEER, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

BiteAnim:;okay
	battle_anim BITE, SUBANIM_02, 0, 8
	db -1 ; end

GrowlAnim:;okay
	battle_anim GROWL, SUBANIM_12, 1, 6
	db -1 ; end

RoarAnim::;okay
	battle_anim ROAR, SUBANIM_15, 1, 6
	battle_anim ROAR, SUBANIM_15, 1, 6
	battle_anim ROAR, SUBANIM_15, 1, 6
	db -1 ; end

SingAnim:;ok
	battle_anim SING, SUBANIM_12, 1, 6
	battle_anim NO_MOVE, SUBANIM_40, 1, 16
	battle_anim NO_MOVE, SUBANIM_40, 1, 16
	db -1 ; end

SupersonicAnim:;done
	battle_anim SUPERSONIC, SUBANIM_31, 0, 4
	battle_anim NO_MOVE, SUBANIM_17, 0, 2
	battle_anim NO_MOVE, SUBANIM_17, 0, 4
	db -1 ; end


SonicBoomAnim:;done
	battle_anim ROAR, SUBANIM_15, 1, 6
	battle_anim ROAR, SUBANIM_15, 1, 6
	battle_anim GUST, SUBANIM_10, 1, 6
	battle_anim NO_MOVE, SUBANIM_2B, 1, 3
	db -1 ; end

DisableAnim:;done
	battle_anim LEECH_SEED, SE_DARK_SCREEN_PALETTE
	battle_anim LEER, SE_DARK_SCREEN_FLASH
	battle_anim LEER, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_23, 0, 3
	db -1 ; end

AcidAnim:;done
	battle_anim ACID, SUBANIM_13, 1, 3
	battle_anim ACID, SUBANIM_14, 1, 6
	db -1 ; end

EmberAnim:;ok
	battle_anim EMBER, SUBANIM_11, 1, 6
	db -1 ; end

FlamethrowerAnim:;ok
	battle_anim FLAMETHROWER, SUBANIM_1F, 1, 6
	battle_anim FLAMETHROWER, SUBANIM_0C, 1, 6
	battle_anim FLAMETHROWER, SUBANIM_0D, 1, 6
	db -1 ; end

MistAnim:;done
	battle_anim NO_MOVE, SE_LIGHT_SCREEN_PALETTE
	battle_anim SURF, SE_WATER_DROPLETS_EVERYWHERE
	battle_anim NO_MOVE, SUBANIM_2F, 0, 8
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

WaterGunAnim:;ok
	battle_anim WATER_GUN, SUBANIM_2C, 0, 6
	db -1 ; end

HydroPumpAnim:;ok
	battle_anim HYDRO_PUMP, SUBANIM_1A, 0, 6
	battle_anim HYDRO_PUMP, SUBANIM_1A, 0, 6
	db -1 ; end

SurfAnim:;ok
	battle_anim SURF, SE_WATER_DROPLETS_EVERYWHERE
	battle_anim HYDRO_PUMP, SUBANIM_1A, 0, 6
	db -1 ; end

IceBeamAnim:;ok
	battle_anim ICE_BEAM, SUBANIM_2E, 0, 3
	battle_anim NO_MOVE, SUBANIM_2F, 0, 16
	db -1 ; end

BlizzardAnim:;ok
	battle_anim BLIZZARD, SUBANIM_38, 0, 4
	battle_anim HYDRO_PUMP, SUBANIM_38, 0, 4
	db -1 ; end

PsyBeamAnim:;ok
	battle_anim PSYBEAM, SUBANIM_2E, 0, 3
	battle_anim NO_MOVE, SE_FLASH_SCREEN_LONG
	db -1 ; end

BubbleBeamAnim:;ok
	battle_anim BUBBLEBEAM, SUBANIM_35, 0, 15
	db -1 ; end

AuroraBeamAnim:;done
	battle_anim NO_MOVE, SE_FLASH_SCREEN_LONG
	battle_anim AURORA_BEAM, SUBANIM_2E, 0, 4
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	db -1 ; end

HyperBeamAnim:;done
	battle_anim LEECH_SEED, SE_DARK_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SPIRAL_BALLS_INWARD
	battle_anim HYPER_BEAM, SUBANIM_2E, 0, 2
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	battle_anim MEGA_PUNCH, SUBANIM_04, 0, 6
	db -1 ; end

PeckAnim:;ok
	battle_anim PECK, SUBANIM_01, 0, 8
	db -1 ; end

DrillPeckAnim:; very done
	battle_anim DRILL_PECK, SUBANIM_58, 0, 3
	battle_anim DRILL_PECK, SUBANIM_58, 0, 3
	db -1 ; end

SubmissionAnim:;very done
	battle_anim NO_MOVE, SE_SLIDE_MON_OFF
	battle_anim SUBMISSION, SUBANIM_59, 0, 3
	battle_anim NO_MOVE, SUBANIM_03, 0, 3
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	db -1 ; end

LowKickAnim:;ok
	battle_anim LOW_KICK, SE_SLIDE_MON_OFF
	battle_anim NO_MOVE, SUBANIM_04, 1, 6
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	db -1 ; end

CounterAnim:;ok
	battle_anim COUNTER, SE_SLIDE_MON_OFF
	battle_anim COUNTER, SUBANIM_04, 1, 6
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	db -1 ; end

SeismicTossAnim:;ok
	battle_anim NO_MOVE, SE_BLINK_ENEMY_MON
	battle_anim BARRAGE, SUBANIM_4E, 1, 1
	battle_anim NO_MOVE, SE_HIDE_ENEMY_MON_PIC
	battle_anim NO_MOVE, SE_SLIDE_MON_OFF
	battle_anim SEISMIC_TOSS, SUBANIM_4F, 1, 2
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	battle_anim SEISMIC_TOSS, SUBANIM_50, 1, 1
	battle_anim NO_MOVE, SE_SHOW_ENEMY_MON_PIC
	battle_anim NO_MOVE, SE_SHAKE_SCREEN
	db -1 ; end

StrengthAnim:;ok
	battle_anim LEECH_SEED, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_RESET_MON_POSITION
	battle_anim ROCK_THROW, SUBANIM_1E, 0, 16
	battle_anim FIRE_PUNCH, SUBANIM_04, 1, 6
	db -1 ; end

AbsorbAnim:;ok
	battle_anim ABSORB, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_21, 0, 4
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_22, 0, 4
	db -1 ; end

MegaDrainAnim:;ok
	battle_anim MEGA_DRAIN, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SUBANIM_21, 0, 6
	battle_anim NO_MOVE, SUBANIM_22, 0, 6
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

LeechSeedAnim:;ok
	battle_anim LEECH_SEED, SUBANIM_1B, 1, 6
	battle_anim STUN_SPORE, SUBANIM_1C, 1, 21
	db -1 ; end

GrowthAnim:;ok
	battle_anim GROWTH, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SPIRAL_BALLS_INWARD
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

RazorLeafAnim:;ok
	battle_anim RAZOR_LEAF, SE_LEAVES_FALLING
	battle_anim SWIFT, SUBANIM_44, 1, 1
	battle_anim RAZOR_WIND, SUBANIM_16, 0, 1
	db -1 ; end

SolarBeamAnim:;done
	battle_anim GROWTH, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SPIRAL_BALLS_INWARD
	battle_anim SOLARBEAM, SUBANIM_2E, 0, 6
	battle_anim NO_MOVE, SUBANIM_04, 0, 5
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

PoisonPowderAnim:;ok
	battle_anim POISONPOWDER, SUBANIM_36, 0, 4
	battle_anim NO_MOVE, SUBANIM_5B, 0, 4
	battle_anim NO_MOVE, SUBANIM_5B, 0, 6
	db -1 ; end

StunSporeAnim:;done
	battle_anim STUN_SPORE, SUBANIM_36, 0, 4
	battle_anim NO_MOVE, SUBANIM_23, 0, 2
	battle_anim NO_MOVE, SUBANIM_23, 0, 4
	db -1 ; end

SleepPowderAnim:;done but is slightly bugged
	battle_anim SLEEP_POWDER, SUBANIM_36, 0, 4
	battle_anim NO_MOVE, SUBANIM_5D, 0, 4
	battle_anim NO_MOVE, SUBANIM_5D, 0, 6
	db -1 ; end

PetalDanceAnim:;done
	battle_anim LEECH_SEED, SE_SLIDE_MON_DOWN
	battle_anim NO_MOVE, SE_SLIDE_MON_UP
	battle_anim NO_MOVE, SE_LIGHT_SCREEN_PALETTE
	battle_anim PETAL_DANCE, SE_PETALS_FALLING
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

StringShotAnim:;OK
	battle_anim STRING_SHOT, SUBANIM_37, 0, 8
	db -1 ; end

DragonRageAnim:;OK
	battle_anim DRAGON_RAGE, SUBANIM_1F, 1, 6
	battle_anim NO_MOVE, SUBANIM_0C, 1, 6
	battle_anim NO_MOVE, SUBANIM_0D, 1, 6
	battle_anim NO_MOVE, SUBANIM_0E, 1, 6
	db -1 ; end

FireSpinAnim:;DONE
	battle_anim FIRE_SPIN, SUBANIM_0C, 1, 2
	battle_anim NO_MOVE, SUBANIM_0D, 1, 3
	battle_anim NO_MOVE, SUBANIM_0E, 1, 4
	db -1 ; end

ThunderShockAnim:;OK
	battle_anim THUNDERSHOCK, SUBANIM_29, 1, 2
	db -1 ; end

ThunderBoltAnim:;OK
	battle_anim THUNDERBOLT, SUBANIM_29, 1, 1
	battle_anim THUNDERBOLT, SUBANIM_29, 1, 1
	db -1 ; end

ThunderWaveAnim:;OK
	battle_anim THUNDER_WAVE, SUBANIM_29, 1, 2
	battle_anim NO_MOVE, SUBANIM_23, 0, 2
	battle_anim NO_MOVE, SUBANIM_23, 0, 4
	db -1 ; end

ThunderAnim:;OK
	battle_anim THUNDER, SE_DARK_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SUBANIM_2B, 1, 6
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim THUNDERBOLT, SUBANIM_29, 1, 2
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

RockThrowAnim:;OK
	battle_anim ROCK_THROW, SUBANIM_30, 0, 4
	db -1 ; end

EarthquakeAnim:;OK
	battle_anim EARTHQUAKE, SE_SHAKE_SCREEN
	battle_anim EARTHQUAKE, SE_SHAKE_SCREEN
	db -1 ; end

FissureAnim:;DONE
	battle_anim FISSURE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_SHAKE_SCREEN
	battle_anim FISSURE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_SHAKE_SCREEN
	battle_anim FISSURE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_SHAKE_SCREEN
	db -1 ; end

DigAnim:;OK
	battle_anim DIG, SUBANIM_04, 1, 6
	battle_anim NO_MOVE, SE_SLIDE_MON_UP
	db -1 ; end

ToxicAnim:;OK
	battle_anim SURF, SE_WATER_DROPLETS_EVERYWHERE
	battle_anim TOXIC, SUBANIM_14, 1, 8
	db -1 ; end

ConfusionAnim:;ok
	battle_anim CONFUSION, SE_FLASH_SCREEN_LONG
	db -1 ; end

PsychicAnim:;ok
	battle_anim PSYCHIC_M, SE_FLASH_SCREEN_LONG
	battle_anim NO_MOVE, SE_WAVY_SCREEN
	db -1 ; end

HypnosisAnim:;ok
	battle_anim HYPNOSIS, SE_FLASH_SCREEN_LONG
	db -1 ; end

MeditateAnim:;ok
	battle_anim MEDITATE, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_43, 1, 6
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

AgilityAnim:; done
	battle_anim QUICK_ATTACK, SE_SHAKE_BACK_AND_FORTH
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	battle_anim AGILITY, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

QuickAttackAnim:;ok
	battle_anim QUICK_ATTACK, SE_SLIDE_MON_OFF
	battle_anim NO_MOVE, SUBANIM_04, 1, 6
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	db -1 ; end

RageAnim:;ok
	battle_anim RAGE, SUBANIM_01, 0, 6
	db -1 ; end

TeleportAnim:;ok
	battle_anim TELEPORT, SE_SQUISH_MON_PIC
	battle_anim NO_MOVE, SE_SHOOT_BALLS_UPWARD
	db -1 ; end

NightShadeAnim:;done
	battle_anim CONFUSION, SE_FLASH_SCREEN_LONG
	battle_anim NO_MOVE, SE_DARK_SCREEN_PALETTE
	battle_anim DREAM_EATER, SE_WAVY_SCREEN
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

MimicAnim:;ok
	battle_anim MIMIC, SUBANIM_21, 1, 6
	battle_anim MIMIC, SUBANIM_22, 1, 6
	db -1 ; end

ScreechAnim:;ok
	battle_anim SCREECH, SUBANIM_12, 1, 6
	db -1 ; end

DoubleTeamAnim:;ok
	battle_anim NO_MOVE, SE_DARK_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	battle_anim DOUBLE_TEAM, SE_SHAKE_BACK_AND_FORTH
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	battle_anim BARRIER, SUBANIM_33, 1, 6
	db -1 ; end

RecoverAnim:;ok
	battle_anim RECOVER, SE_BLINK_MON
	battle_anim NO_MOVE, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SPIRAL_BALLS_INWARD
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

HardenAnim:;ok
	battle_anim HARDEN, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_43, 1, 6
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

MinimizeAnim:;ok
	battle_anim MINIMIZE, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SPIRAL_BALLS_INWARD
	battle_anim NO_MOVE, SE_MINIMIZE_MON
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

SmokeScreenAnim:;ok
	battle_anim SMOKESCREEN, SUBANIM_28, 1, 6
	battle_anim NO_MOVE, SUBANIM_0A, 0, 4
	battle_anim NO_MOVE, SE_DARKEN_MON_PALETTE
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DARK_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_DARKEN_MON_PALETTE
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

ConfuseRayAnim:;ok
	battle_anim CONFUSE_RAY, SE_DARK_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_3E, 1, 6
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

WithdrawAnim:;ok
	battle_anim DEFENSE_CURL, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_51, 0, 6
	battle_anim WATER_GUN, SUBANIM_1A, 0, 4
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SLIDE_MON_UP
	db -1 ; end

DefenseCurlAnim:;ok
	battle_anim DEFENSE_CURL, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_43, 0, 6
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

BarrierAnim:;ok
	battle_anim BARRIER, SUBANIM_33, 1, 6
	battle_anim BARRIER, SUBANIM_33, 1, 6
	db -1 ; end

LightScreenAnim:;ok
	battle_anim NO_MOVE, SE_LIGHT_SCREEN_PALETTE
	battle_anim LIGHT_SCREEN, SUBANIM_33, 1, 6
	battle_anim LIGHT_SCREEN, SUBANIM_33, 1, 6
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

HazeAnim:;ok
	battle_anim NO_MOVE, SE_DARKEN_MON_PALETTE
	battle_anim SURF, SE_WATER_DROPLETS_EVERYWHERE
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

ReflectAnim:;ok
	battle_anim NO_MOVE, SE_DARK_SCREEN_PALETTE
	battle_anim REFLECT, SUBANIM_33, 1, 6
	battle_anim REFLECT, SUBANIM_33, 1, 6
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

FocusEnergyAnim:;done
	battle_anim FOCUS_ENERGY, SE_SPIRAL_BALLS_INWARD
	battle_anim NO_MOVE, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

BideAnim:;ok
	battle_anim BIDE, SUBANIM_04, 1, 6
	db -1 ; end

MetronomeAnim:;OK
	battle_anim AMNESIA, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim AMNESIA, SE_RESET_MON_POSITION
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim AMNESIA, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_DELAY_ANIMATION_10
	battle_anim AMNESIA, SE_RESET_MON_POSITION
	db -1 ; end

MirrorMoveAnim:;ok
	battle_anim MIRROR_MOVE, SUBANIM_01, 0, 8
	db -1 ; end

SelfdestructAnim:;ok but check I can alter the megapunch afterwards
	battle_anim SELFDESTRUCT, SUBANIM_34, 1, 3
	db -1 ; end

EggBombAnim:;done
	battle_anim EGG_BOMB, SUBANIM_41, 1, 4
	battle_anim EGG_BOMB, SUBANIM_42, 1, 4
	battle_anim ROCK_THROW, SUBANIM_30, 0, 4
	db -1 ; end

LickAnim:;ok
	battle_anim SLUDGE, SUBANIM_59, 0, 4
	battle_anim NO_MOVE, SUBANIM_14, 1, 4
	db -1 ; end

SmogAnim:;ok
	battle_anim LEECH_SEED, SE_DARKEN_MON_PALETTE
	battle_anim SMOG, SUBANIM_19, 1, 6
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

SludgeAnim:;
	battle_anim SLUDGE, SUBANIM_13, 1, 8
	battle_anim SLUDGE, SUBANIM_14, 1, 3
	battle_anim NO_MOVE, SUBANIM_14, 1, 3
	db -1 ; end

BoneClubAnim:;done
	battle_anim BONE_CLUB, SUBANIM_5E, 0, 3
	battle_anim NO_MOVE, SUBANIM_01, 0, 4
	db -1 ; end

FireBlastAnim:;ok
	battle_anim FIRE_BLAST, SUBANIM_1F, 1, 6
	battle_anim NO_MOVE, SUBANIM_20, 1, 6
	battle_anim NO_MOVE, SUBANIM_20, 1, 6
	battle_anim NO_MOVE, SUBANIM_0C, 1, 6
	battle_anim NO_MOVE, SUBANIM_0D, 1, 6
	db -1 ; end

WaterfallAnim:;done
	battle_anim LEECH_SEED, SE_SLIDE_MON_DOWN
	battle_anim HYDRO_PUMP, SUBANIM_59, 0, 4
	battle_anim NO_MOVE, SUBANIM_1A, 0, 4
	battle_anim NO_MOVE, SE_SLIDE_MON_UP
	db -1 ; end

ClampAnim:;ok
	battle_anim CLAMP, SUBANIM_2A, 0, 8
	battle_anim CONSTRICT, SUBANIM_23, 0, 6
	battle_anim CONSTRICT, SUBANIM_23, 0, 6
	db -1 ; end

SwiftAnim:;ok
	battle_anim SWIFT, SUBANIM_3F, 1, 3
	db -1 ; end

SkullBashAnim:;done
	battle_anim SKULL_BASH, SUBANIM_04, 1, 6
	battle_anim NO_MOVE, SE_SHAKE_SCREEN
	db -1 ; end

SpikeCannonAnim:;very done
	battle_anim SPIKE_CANNON, SUBANIM_56, 0, 3
	battle_anim NO_MOVE, SUBANIM_2F, 0, 4
	db -1 ; end

ConstrictAnim:;done
	battle_anim STRING_SHOT, SUBANIM_37, 0, 4
	battle_anim CONSTRICT, SUBANIM_23, 0, 2
	battle_anim CONSTRICT, SUBANIM_23, 0, 6
	battle_anim CONSTRICT, SUBANIM_23, 0, 10
	db -1 ; end

AmnesiaAnim:;ok for whatever reason doesn't let me edit what the rival does
	battle_anim AMNESIA, SUBANIM_25, 0, 8
	battle_anim AMNESIA, SUBANIM_25, 0, 8
	db -1 ; end

KinesisAnim:;done
	battle_anim KINESIS, SUBANIM_01, 0, 8
	battle_anim LEER, SE_WAVY_SCREEN
	db -1 ; end

SoftboiledAnim:;ok
	battle_anim LEECH_SEED, SE_SLIDE_MON_HALF_OFF
	battle_anim SOFTBOILED, SUBANIM_4C, 0, 8
	battle_anim NO_MOVE, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SPIRAL_BALLS_INWARD
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	db -1 ; end

HiJumpKickAnim:;very done
	battle_anim LEECH_SEED, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_2D, 0, 3
	battle_anim NO_MOVE, SE_SLIDE_MON_HALF_OFF
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_MOVE_MON_HORIZONTALLY
	battle_anim NO_MOVE, SE_RESET_MON_POSITION
	battle_anim HI_JUMP_KICK, SUBANIM_04, 1, 6
	db -1 ; end

GlareAnim:;ok
	battle_anim LEECH_SEED, SE_DARK_SCREEN_PALETTE
	battle_anim GLARE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

DreamEaterAnim:;ok
	battle_anim DREAM_EATER, SE_FLASH_SCREEN_LONG
	battle_anim DREAM_EATER, SE_DARK_SCREEN_PALETTE
	battle_anim DREAM_EATER, SUBANIM_02, 0, 8
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

PoisonGasAnim:;done
	battle_anim NO_MOVE, SE_DARK_SCREEN_PALETTE
	battle_anim POISON_GAS, SUBANIM_19, 1, 6
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	battle_anim SMOG, SUBANIM_5B, 0, 6
	battle_anim SMOG, SUBANIM_5B, 0, 8

	db -1 ; end

BarrageAnim:;done
	battle_anim BARRAGE, SUBANIM_41, 1, 3
	battle_anim NO_MOVE, SUBANIM_55, 0, 3
	db -1 ; end

LeechLifeAnim:;done
	battle_anim LEECH_LIFE, SUBANIM_02, 0, 6
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SUBANIM_21, 0, 4
	battle_anim NO_MOVE, SUBANIM_22, 0, 4
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	db -1 ; end

LovelyKissAnim:;done
	battle_anim LOVELY_KISS, SUBANIM_12, 0, 4
	battle_anim LOVELY_KISS, SUBANIM_40, 0, 6
	db -1 ; end

SkyAttackAnim:;done
	battle_anim SKY_ATTACK, SE_SQUISH_MON_PIC
	battle_anim NO_MOVE, SE_SHOOT_BALLS_UPWARD
	battle_anim HI_JUMP_KICK, SUBANIM_04, 1, 6
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	db -1 ; end

TransformAnim:;ok
	battle_anim TRANSFORM, SUBANIM_21, 1, 6
	battle_anim TRANSFORM, SUBANIM_22, 1, 4
	battle_anim NO_MOVE, SUBANIM_47, 0, 8
	battle_anim NO_MOVE, SE_TRANSFORM_MON
	db -1 ; end

BubbleAnim:;ok
	battle_anim BUBBLE, SUBANIM_35, 0, 22
	db -1 ; end

DizzyPunchAnim:;done
	battle_anim COMET_PUNCH, SUBANIM_17, 0, 4
	battle_anim COMET_PUNCH, SUBANIM_17, 0, 4
	battle_anim DOUBLESLAP, SUBANIM_02, 0, 4
	battle_anim NO_MOVE, SUBANIM_21, 0, 4
	battle_anim NO_MOVE, SUBANIM_22, 0, 4
	db -1 ; end

SporeAnim:;VERY DONE
	battle_anim SPORE, SUBANIM_36, 0, 4
	battle_anim REST, SUBANIM_5C, 0, 4
	db -1 ; end

FlashAnim:;ok
	battle_anim LEECH_SEED, SE_LIGHT_SCREEN_PALETTE
	battle_anim GLARE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

PsywaveAnim:;VERY DONE
	battle_anim SUPERSONIC, SUBANIM_31, 0, 6
	battle_anim KINESIS, SUBANIM_42, 1, 4
	battle_anim NO_MOVE, SE_DARK_SCREEN_PALETTE
	battle_anim PSYBEAM, SE_WAVY_SCREEN
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

SplashAnim:;very done
	battle_anim SKY_ATTACK, SE_BOUNCE_UP_AND_DOWN
	battle_anim BLIZZARD, SE_DARKEN_MON_PALETTE
	battle_anim NO_MOVE, SUBANIM_10, 1, 1
	battle_anim NO_MOVE, SUBANIM_10, 1, 1
	battle_anim NO_MOVE, SUBANIM_10, 1, 1
	battle_anim NO_MOVE, SUBANIM_04, 1, 4
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

AcidArmorAnim:;done
	battle_anim ACID_ARMOR, SE_SLIDE_MON_DOWN_AND_HIDE
	battle_anim HARDEN, SE_SHOW_MON_PIC
	db -1 ; end

CrabHammerAnim:;done
	battle_anim CRABHAMMER, SUBANIM_2A, 0, 3
	battle_anim NO_MOVE, SUBANIM_5A, 1, 4
	battle_anim HYDRO_PUMP, SUBANIM_1A, 0, 4
	db -1 ; end

ExplosionAnim:;ok but check I can alter the megapunch afterwards
	battle_anim EXPLOSION, SUBANIM_34, 1, 4
	db -1 ; end

FurySwipesAnim:; ok
	battle_anim FURY_SWIPES, SUBANIM_0F, 0, 4
	db -1 ; end

BonemerangAnim:; very done
	battle_anim BONEMERANG, SUBANIM_5E, 0, 2
	battle_anim NO_MOVE, SUBANIM_02, 0, 4
	battle_anim NO_MOVE, SUBANIM_5F, 0, 2
	db -1 ; end

RestAnim:;ok
	battle_anim REST, SUBANIM_3A, 0, 16
	battle_anim REST, SUBANIM_3A, 0, 16
	db -1 ; end

RockSlideAnim:;ok
	battle_anim ROCK_SLIDE, SUBANIM_1D, 0, 4
	battle_anim ROCK_SLIDE, SUBANIM_1E, 0, 3
	battle_anim HYPER_FANG, SUBANIM_04, 1, 6
	db -1 ; end

HyperFangAnim:;done
	battle_anim HYPER_FANG, SUBANIM_02, 0, 4
	battle_anim HYPER_FANG, SUBANIM_02, 0, 4
	db -1 ; end

SharpenAnim:;done
	battle_anim SHARPEN, SUBANIM_43, 1, 4
	battle_anim SHARPEN, SUBANIM_43, 1, 4	
	db -1 ; end

ConversionAnim:;done
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	battle_anim CONVERSION, SUBANIM_21, 1, 4
	battle_anim NO_MOVE, SUBANIM_22, 1, 2
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	db -1 ; end

TriAttackAnim:;ok
	battle_anim TRI_ATTACK, SE_DARK_SCREEN_FLASH
	battle_anim NO_MOVE, SUBANIM_4D, 1, 6
	battle_anim NO_MOVE, SE_DARK_SCREEN_FLASH
	db -1 ; end

SuperFangAnim:;done
	battle_anim LEECH_SEED, SE_DARK_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SHAKE_SCREEN
	battle_anim SUPER_FANG, SUBANIM_04, 1, 6
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

SlashAnim:;done
	battle_anim CUT, SUBANIM_16, 0, 2
	battle_anim SLASH, SUBANIM_0F, 0, 8
	db -1 ; end

SubstituteAnim:;ok
	battle_anim SUBSTITUTE, SE_SLIDE_MON_OFF
	battle_anim NO_MOVE, SUBANIM_47, 0, 8
	battle_anim NO_MOVE, SE_SUBSTITUTE_MON
	db -1 ; end

BallTossAnim:
	battle_anim NO_MOVE, SUBANIM_06, 0, 3
	db -1 ; end

GreatTossAnim:
	battle_anim NO_MOVE, SUBANIM_07, 0, 3
	db -1 ; end

UltraTossAnim:
	battle_anim NO_MOVE, SUBANIM_08, 0, 2
	db -1 ; end

BallShakeAnim:
	battle_anim NO_MOVE, SUBANIM_09, 0, 4
	db -1 ; end

BallPoofAnim:
	battle_anim NO_MOVE, SUBANIM_0A, 0, 4
	db -1 ; end

ShowPicAnim:
	battle_anim NO_MOVE, SE_SHOW_ENEMY_MON_PIC
	db -1 ; end

HidePicAnim:
	battle_anim NO_MOVE, SE_HIDE_ENEMY_MON_PIC
	db -1 ; end

EnemyFlashAnim:
	battle_anim NO_MOVE, SE_SHOW_MON_PIC
	db -1 ; end

PlayerFlashAnim:
	battle_anim NO_MOVE, SE_FLASH_MON_PIC
	db -1 ; end

EnemyHUDShakeAnim:
	battle_anim NO_MOVE, SE_SHAKE_ENEMY_HUD
	db -1 ; end

TradeBallDropAnim:
	battle_anim NO_MOVE, SUBANIM_48, 2, 6
	db -1 ; end

TradeBallAppear1Anim:
	battle_anim NO_MOVE, SUBANIM_49, 2, 4
	db -1 ; end

TradeBallAppear2Anim:
	battle_anim NO_MOVE, SUBANIM_4A, 2, 6
	db -1 ; end

TradeBallPoofAnim:
	battle_anim NO_MOVE, SUBANIM_4B, 2, 6
	db -1 ; end

XStatItemAnim:
	battle_anim NO_MOVE, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SPIRAL_BALLS_INWARD
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

ShrinkingSquareAnim:
	battle_anim NO_MOVE, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SUBANIM_43, 1, 6
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

XStatItemBlackAnim:
	battle_anim NO_MOVE, SE_DARKEN_MON_PALETTE
	battle_anim NO_MOVE, SE_SPIRAL_BALLS_INWARD
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

ShrinkingSquareBlackAnim:
	battle_anim NO_MOVE, SE_DARKEN_MON_PALETTE
	battle_anim NO_MOVE, SUBANIM_43, 1, 6
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

UnusedAnim:
	battle_anim NO_MOVE, SE_LIGHT_SCREEN_PALETTE
	battle_anim NO_MOVE, SE_SHOOT_MANY_BALLS_UPWARD
	battle_anim NO_MOVE, SE_RESET_SCREEN_PALETTE
	db -1 ; end

ParalyzeAnim:
	battle_anim BIND, SUBANIM_24, 0, 4
	battle_anim BIND, SUBANIM_24, 0, 4
	db -1 ; end

PoisonAnim:
	battle_anim BIND, SUBANIM_27, 0, 8
	battle_anim BIND, SUBANIM_27, 0, 8
	db -1 ; end

SleepPlayerAnim:
	battle_anim REST, SUBANIM_3A, 0, 16
	battle_anim REST, SUBANIM_3A, 0, 16
	db -1 ; end

SleepEnemyAnim:
	battle_anim REST, SUBANIM_3B, 0, 16
	battle_anim REST, SUBANIM_3B, 0, 16
	db -1 ; end

ConfusedPlayerAnim:
	battle_anim AMNESIA, SUBANIM_25, 0, 8
	battle_anim AMNESIA, SUBANIM_25, 0, 8
	db -1 ; end

ConfusedEnemyAnim:
	battle_anim AMNESIA, SUBANIM_26, 0, 8
	battle_anim AMNESIA, SUBANIM_26, 0, 8
	db -1 ; end

BallBlockAnim:
	battle_anim NO_MOVE, SUBANIM_0B, 0, 3
	db -1 ; end

FaintAnim:
	battle_anim DIG, SE_SLIDE_MON_DOWN
	db -1 ; end

ShakeScreenAnim:
	battle_anim NO_MOVE, SE_SHAKE_SCREEN
	db -1 ; end

ThrowRockAnim:
	battle_anim BARRAGE, SUBANIM_53, 0, 3
	db -1 ; end

ThrowBaitAnim:
	battle_anim BARRAGE, SUBANIM_54, 0, 3
	db -1 ; end
